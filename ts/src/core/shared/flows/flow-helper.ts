...
function deleteAction(flowVersion: FlowVersion, request: DeleteActionRequest): FlowVersion {
...
function updateAction(flowVersion: FlowVersion, request: UpdateActionRequest): FlowVersion {
  return transferFlow(flowVersion, (parentStep) => {
    if (parentStep.nextAction && parentStep.nextAction.name === request.name) {
...
function moveAction(flowVersion: FlowVersion, request: MoveActionRequest): FlowVersion {
  ...
  return transferFlow(flowVersion, (parentStep: Step) => {
    ...
    if (parentStep.type === ActionType.LOOP_ON_ITEMS && request.stepLocationRelativeToParent) {
      ...
      if (
        ...
        request.stepLocationRelativeToParent === StepLocationRelativeToParent.INSIDE_FALSE_BRANCH
      ) {
...
export function getImportOperations(step: Action | Trigger | undefined): FlowOperationRequest[] {
...
function isStepLastChildOfParent(child: Step, trigger: Trigger): boolean {
  ...
  if (parent) {
    if (doesStepHaveChildren(parent)) {
      ...
      return (
        trueBranchChildren[trueBranchChildren.length - 1]?.name === child.name ||
  ...
  apply(flowVersion: FlowVersion, operation: FlowOperationRequest): FlowVersion {
    ...
    switch (operation.type) {
      ...
      case FlowOperationType.ADD_ACTION: {
        clonedVersion = transferFlow(addAction(clonedVersion, operation.request), (step) =>
          upgradePiece(step, operation.request.action.name)
        );
        break;
      }
      case FlowOperationType.UPDATE_ACTION:
        clonedVersion = transferFlow(updateAction(clonedVersion, operation.request), (step) =>
