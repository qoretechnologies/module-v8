...
const AmazonSQSNewMessageTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    await pollCreatedItemsForTrigger({
      trigger_name: 'amazon_sqs_new_message',
  ...
  event_info: {
    desc: 'Amazon SQS New Message Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
...
export default AmazonSQSNewMessageTrigger;
...
const fetchLatestMessages = async (options: {
  ...
  const { access_key_id, secret_access_key, region, queue_url, wait_time_seconds, max_messages } = options;
  ...
  try {
    ...
    throw new AmazonSQSError(`Failed to fetch latest messages: ${error.message || error}`);
  }
};
