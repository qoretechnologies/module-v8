...
const createJSONMessage = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    const { access_key_id, secret_access_key, queue_url, message_data } =
      getQoreContextRequiredValues({
        context: { ...context, opts: obj },
        connectionFields: ['access_key_id', 'secret_access_key'],
        optionFields: ['queue_url', 'message_data'],
    ...
    try {
      messageBody = JSON.stringify(message_data);
    } catch (error) {
      throw new AmazonSQSError(
        `Failed to serialize message data to JSON: ${error.message || error}`
      );
    }
    ...
    try {
      ...
      throw new AmazonSQSError(`Failed to create JSON message: ${error.message || error}`);
    }
  },
  response_type: {
    type: 'hash',
