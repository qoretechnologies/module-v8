...
const GoogleDriveNewFileTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    const file_types = context.opts?.file_types || [];
...
const fetchLatestFiles = async (
  ...
  try {
    ...
    const file_types = options.file_types || [];
    ...
    throw new GoogleDriveError(`Failed to fetch latest files: ${error.message || error}`);
  }
};
...
const fetchFileContent = async (token: string, file: Record<string, any>) => {
  try {
    ...
    throw new GoogleDriveError(`Failed to fetch file content: ${error.message || error}`);
  }
};

export default GoogleDriveNewFileTrigger;
