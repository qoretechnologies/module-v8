...
const action = 'add_contacts_to_customer_list';
...
const addContactsToCustomerList = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      ...
      throw new GoogleAdsError(`Failed to add contacts to customer list: ${message}`);
    }
  },
  response_type: {
    type: 'hash',
    fields: {
      ...
      contacts_added: { type: 'integer' },
      status: { type: 'string' },
    },
  },
});

export default addContactsToCustomerList;
