...
export const getOutlookEmailAllowedValues: TQoreGetAllowedValuesFunction<
  TCustomConnOptions,
  any
> = async (context): Promise<IQoreAllowedValue<any>[]> => {
  ...
  if (missingValues.length) {
    throw new Error(
      `All of the following ${missingValues.join(', ')} are required to get Outlook email allowed values`
    );
  }
