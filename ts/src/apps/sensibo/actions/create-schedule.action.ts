import { EQoreAppActionCode, QoreAppCreator, TQoreOptions } from '@qoretechnologies/ts-toolkit';
import { getQoreContextRequiredValues } from '../../../global/helpers';
import { SENSIBO_APP_NAME, SensiboError } from '../constants';
import { sensiboApiClient } from '../helpers/constants';
import { getSensiboDeviceAllowedValues } from '../helpers/get-device-allowed-values';

const options = {
  device: {
    type: 'string',
    required: true,
    get_allowed_values: getSensiboDeviceAllowedValues,
  },
  targetTimeLocale: {
    type: 'string',
    required: true,
  },
  timezone: {
    type: 'string',
    required: true,
  },
  acState: {
    type: {
      type: 'hash',
      fields: {
        on: { type: 'boolean', required: true },
        mode: { type: 'string', required: true },
        fanLevel: { type: 'string', required: true },
        targetTemperature: { type: 'number', required: true },
        temperatureUnit: { type: 'string', required: true },
        swing: { type: 'string', required: true },
      },
    },
  },
  recurOnDaysOfWeek: {
    type: 'string',
    required: true,
  },
} satisfies TQoreOptions;

const createSchedule = QoreAppCreator.createLocalizedAction<typeof options>({
  app: SENSIBO_APP_NAME,
  options,
  action: 'create_schedule',
  action_code: EQoreAppActionCode.ACTION,
  api_function: async (obj, _opts, context) => {
    const { token, device, ...data } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      connectionFields: ['token'],
      optionFields: ['device', 'targetTimeLocale', 'timezone', 'acState', 'recurOnDaysOfWeek'],
      ErrorClass: SensiboError,
    });

    try {
      return await sensiboApiClient({
        token,
        method: 'POST',
        body: data,
        path: `pods/${device}/schedules`,
      });
    } catch (error) {
      throw new SensiboError(`Failed to create the schedule: ${error}`);
    }
  },
  response_type: {
    type: 'hash',
  },
});

export default createSchedule;
