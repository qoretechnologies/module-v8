...
import { getTodoistCollaboratorAllowedValues } from '../helpers/get-collaborator-allowed-values';
...
const options = {
  ...
  assignee_id: {
    type: 'string',
    required: false,
    get_allowed_values: getTodoistCollaboratorAllowedValues,
...
const createTask = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    const { token, content } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      connectionFields: ['token'],
      optionFields: ['content'],
      ErrorClass: TodoistError,
    });

    const { description, due_datetime, duration, labels, order, priority, project_id, section_id } =
      obj || {};
