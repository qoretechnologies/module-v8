...
const CreateRecord = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    const { token, module, properties, url } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      connectionFields: ['url', 'token'],
      optionFields: ['module', 'properties'],
    ...
    try {
      ...
      if (!isSuccess(recordData.status)) {
        throw new ZohoCrmError(
          `Failed to create record in module ${module}: ${recordData.message} (code: ${recordData.code})`
        );
      }
