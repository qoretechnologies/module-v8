...
const action = 'capture_order';

const captureOrder = QoreAppCreator.createLocalizedAction<typeof options>({
  app: PAYPAL_APP_NAME,
  action,
  ...
  api_function: async (obj, _opts, context) => {
    ...
    const captureBody: Record<string, any> = {
      ...(obj?.final_capture !== undefined && { final_capture: obj.final_capture }),
      ...(obj?.payment_instruction && { payment_instruction: obj.payment_instruction }),
    };
    ...
    try {
      const response = await payPalApiClient<Record<string, any>>({
        ...
        method: 'POST',
        body: Object.keys(captureBody).length > 0 ? captureBody : undefined,
        token,
        environment,
      });
