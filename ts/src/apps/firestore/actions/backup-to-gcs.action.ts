...
const backupToGCS = QoreAppCreator.createLocalizedAction<typeof options>({
  app: FIRESTORE_APP_NAME,
  action: 'backup_to_google_cloud_storage',
  action_code: EQoreAppActionCode.ACTION,
  options,
  api_function: async (obj, _opts, context) => {
    const { token, project_id, output_uri_prefix } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      optionFields: ['project_id', 'output_uri_prefix'],
    ...
    try {
      ...
      throw new FirestoreError(`Failed to backup to GCS: ${getFirestoreErrorMessage(error)}`);
    }
  },
  response_type: {
    type: 'hash',
