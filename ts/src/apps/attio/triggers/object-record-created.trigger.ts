...
const attioObjectRecordCreatedTrigger = QoreAppCreator.createLocalizedTrigger({
  app: ATTIO_APP_NAME,
  action: 'object_record_created',
  ...
  webhook_register: async (context, url) => {
    ...
    try {
      ...
      throw new AttioError(`Failed to register webhook for object record created event ${error}`);
    }
  },
  webhook_deregister: deregisterAttioWebhook,
  get_example_event_data: createAttioObjectRecordExampleEventData('created'),
  event_info: getAttioObjectRecordEventEventInfo('created'),
});

export default attioObjectRecordCreatedTrigger;
