...
const attioObjectRecordDeletedTrigger = QoreAppCreator.createLocalizedTrigger({
  app: ATTIO_APP_NAME,
  action: 'object_record_deleted',
  ...
  webhook_register: async (context, url) => {
    ...
    try {
      ...
      throw new AttioError(`Failed to register webhook for object record deleted event ${error}`);
    }
  },
  webhook_deregister: deregisterAttioWebhook,
  get_example_event_data: createAttioObjectRecordExampleEventData('deleted'),
  event_info: getAttioObjectRecordEventEventInfo('deleted'),
});

export default attioObjectRecordDeletedTrigger;
