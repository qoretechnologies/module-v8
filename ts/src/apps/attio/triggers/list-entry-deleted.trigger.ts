...
const attioListEntryDeletedTrigger = QoreAppCreator.createLocalizedTrigger({
  app: ATTIO_APP_NAME,
  ...
  webhook_register: async (context, url) => {
    ...
    try {
      ...
      throw new AttioError(`Failed to register webhook for list entry deleted event ${error}`);
    }
  },
  webhook_deregister: deregisterAttioWebhook,
  get_example_event_data: createAttioListEntryExampleEventData('deleted'),
  event_info: getAttioListEntryEventEventInfo('deleted'),
});

export default attioListEntryDeletedTrigger;
