...
const pollMailchimpUnsubscribers = async (options: {
  trigger_name: string;
  token: string;
  datacenter: string;
  audienceId: string;
  ...
  try {
    ...
    const processedUnsubscribers = new Set<string>();
    ...
    initialMembers.members.forEach((member) => {
      processedUnsubscribers.add(member.email_address.toLowerCase());
    });

    Debugger.log(
      `[${trigger_name}] Initialized with ${processedUnsubscribers.size} existing unsubscribers`
    );
    ...
    while (!should_stop()) {
      try {
        ...
        for (const member of currentMembers.members) {
          ...
          if (!processedUnsubscribers.has(emailAddress)) {
            const eventData = {
              type: 'unsubscribed',
              list_id: audienceId,
              member: {
            ...
            processedUnsubscribers.add(emailAddress);

            Debugger.log(`[${trigger_name}] New unsubscriber detected: ${member.email_address}`);
          }
        }
...
const mailchimpNewUnsubscriberTrigger = QoreAppCreator.createLocalizedTrigger({
  app: MAILCHIMP_APP_NAME,
  action: 'new_unsubscriber',
  ...
  event_function: async (context, update, should_stop) => {
    ...
    if (missingValues.length) {
      throw new MailchimpError(
        `All of the following [${missingValues.join(', ')}] are required to start the New Unsubscriber trigger`
      );
    }

    await pollMailchimpUnsubscribers({
      trigger_name: 'mailchimp_new_unsubscriber',
      token: token!,
  ...
  get_example_event_data: async (context) => {
    ...
    if (token && datacenter && audienceId) {
      try {
        const members = await getMailchimpListMembers(
          token,
          datacenter,
          audienceId,
          'unsubscribed'
  ...
  event_info: {
    desc: 'New unsubscriber event data from Mailchimp',
    type: {
      type: 'hash',
      fields: {
        type: { type: 'string' },
...
export default mailchimpNewUnsubscriberTrigger;
