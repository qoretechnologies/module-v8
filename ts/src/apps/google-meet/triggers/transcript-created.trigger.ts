...
const GoogleMeetTranscriptCreatedTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    const getItems = () => {
      return fetchLatestTranscripts(token, conference);
    };

    await pollCreatedItemsForTrigger({
      trigger_name: 'google_meet_transcript_created',
      uniqueField: 'name',
      getItems,
      update,
      should_stop,
  ...
  get_example_event_data: async (context) => {
    ...
    const responses = await fetchLatestTranscripts(token, conference);

    return responses?.length > 0 ? responses[0] : null;
  },
  event_info: {
    desc: 'Triggered when a new Google Meet transcript is created.',
    type: {
      type: 'hash',
      fields: {
        name: {
...
export default GoogleMeetTranscriptCreatedTrigger;
...
const fetchLatestTranscripts = async (token: string, conference_id?: string) => {
  ...
  try {
    ...
    return (response.data.transcripts || []).filter(
      (transcript) => transcript.state === 'FILE_GENERATED'
    );
  } catch (error) {
    throw new GoogleMeetError(`Failed to fetch latest transcripts: ${error.message || error}`);
  }
};
