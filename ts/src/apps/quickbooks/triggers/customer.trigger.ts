...
const QuickbooksCustomerTrigger = QoreAppCreator.createLocalizedTrigger({
  app: QUICKBOOKS_APP_NAME,
  action: 'customer_trigger',
  ...
  event_function: async (context, update, should_stop) => {
    ...
    if (action === 'created') {
      await pollCreatedItemsForTrigger({
        trigger_name: 'quickbooks_customer',
        uniqueField: 'Id',
        getItems,
        update,
        should_stop,
  ...
  event_info: {
    desc: 'Quickbooks Customer Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
        Taxable: { type: 'bool' },
...
export default QuickbooksCustomerTrigger;
...
const fetchCustomers = async (options: {
  ...
  try {
    ...
    throw new QuickbooksError(
      `Failed to fetch latest customers: ${getQuickbooksErrorMessage(error)}`
    );
  }
};
