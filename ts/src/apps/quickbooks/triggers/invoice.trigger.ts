...
const QuickbooksInvoiceTrigger = QoreAppCreator.createLocalizedTrigger({
  app: QUICKBOOKS_APP_NAME,
  action: 'invoice_trigger',
  ...
  event_function: async (context, update, should_stop) => {
    ...
    if (action === 'created') {
      await pollCreatedItemsForTrigger({
        trigger_name: 'quickbooks_invoice',
        uniqueField: 'Id',
        getItems,
        update,
        should_stop,
  ...
  event_info: {
    desc: 'Quickbooks Invoice Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
...
export default QuickbooksInvoiceTrigger;
...
const fetchInvoices = async (options: {
  ...
  try {
    ...
    const response = await client.findInvoices({
      limit,
    ...
    throw new QuickbooksError(
      `Failed to fetch latest invoices: ${getQuickbooksErrorMessage(error)}`
    );
  }
};
