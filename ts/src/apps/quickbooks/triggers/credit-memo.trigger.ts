...
const QuickbooksCreditMemoTrigger = QoreAppCreator.createLocalizedTrigger({
  app: QUICKBOOKS_APP_NAME,
  action: 'credit_memo_trigger',
  ...
  event_function: async (context, update, should_stop) => {
    ...
    if (action === 'created') {
      await pollCreatedItemsForTrigger({
        trigger_name: 'quickbooks_credit_memo',
        uniqueField: 'Id',
        getItems,
        update,
        should_stop,
  ...
  get_example_event_data: async (context) => {
    ...
    const creditMemos = await fetchCreditMemos({
      token,
      instance_type,
      realm_id,
      action,
    ...
    return creditMemos?.length > 0 ? creditMemos[0] : null;
  },
  event_info: {
    desc: 'Quickbooks Credit Memo Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
...
export default QuickbooksCreditMemoTrigger;

const fetchCreditMemos = async (options: {
  token: string;
  ...
  try {
    ...
    throw new QuickbooksError(
      `Failed to fetch latest credit memos: ${getQuickbooksErrorMessage(error)}`
    );
  }
};
