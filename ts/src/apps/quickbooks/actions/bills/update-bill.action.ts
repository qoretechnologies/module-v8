...
const updateBill = QoreAppCreator.createLocalizedAction<
...
>({
  ...
  api_function: async (obj, _opts, context) => {
    const { token, realm_id, instance_type, bill_id } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      connectionFields: ['token', 'realm_id', 'instance_type'],
      optionFields: ['bill_id'],
      ErrorClass: QuickbooksError,
    });

    const line_items = obj?.line_items as Array<ItemBasedLineItem | AccountBasedLineItem>;
    const vendor_id = obj?.vendor_id;
    const line_item_type = obj?.line_item_type;
