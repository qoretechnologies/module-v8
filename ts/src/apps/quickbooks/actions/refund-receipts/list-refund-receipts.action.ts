...
import { QuickBooksRefundReceiptFieldsAllowedValues } from '../../helpers/get-refund-receipt-fields-allowed-values';
    ...
    type: {
      ...
      fields: {
        field: {
          ...
          allowed_values: QuickBooksRefundReceiptFieldsAllowedValues,
        },
        operator: {
          type: 'string',
          required: false,
...
const listRefundReceipts = QoreAppCreator.createLocalizedAction<typeof options>({
  app: QUICKBOOKS_APP_NAME,
  action: 'list_refund_receipts',
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      ...
      return {
        ...
        refund_receipts: response.QueryResponse.RefundReceipt || [],
      };
    } catch (error) {
      throw new QuickbooksError(`Failed to list refund receipts: ${error.message || error}`);
    }
  },
  response_type: {
    type: 'hash',
...
export default listRefundReceipts;
