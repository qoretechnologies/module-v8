...
const createJsonMessage = QoreAppCreator.createLocalizedAction<typeof options>({
  app: AMAZON_SNS_APP_NAME,
  action: 'create_json_message',
  action_code: EQoreAppActionCode.ACTION,
  options,
  api_function: async (obj, _opts, context) => {
    const { access_key_id, secret_access_key, topic_arn, json_message } =
      getQoreContextRequiredValues({
        context: { ...context, opts: obj },
        connectionFields: ['access_key_id', 'secret_access_key'],
        optionFields: ['topic_arn', 'json_message'],
    ...
    try {
      ...
      const messageString = JSON.stringify(json_message);
      const structured = { default: messageString };

      if (messageString.length > SNS_MESSAGE_BODY_LIMIT) {
        throw new AmazonSNSError(
          `JSON message body exceeds maximum size of ${SNS_MESSAGE_BODY_LIMIT} bytes`
        );
      }
      ...
      return {
        ...
        json_message,
        message_string:
          messageString.length > 500 ? messageString.substring(0, 500) + '...' : messageString,
        full_message_length: messageString.length,
        subject: subject || '',
      ...
      throw new AmazonSNSError(`Failed to create JSON message: ${error.message || error}`);
    }
  },
  response_type: {
    type: 'hash',
