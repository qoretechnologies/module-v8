...
const YouTubeNewPlaylistVideoTrigger = QoreAppCreator.createLocalizedTrigger({
  app: YOUTUBE_APP_NAME,
  action: 'new_playlist_video',
  ...
  event_function: async (context, update, should_stop) => {
    const { token, playlist } = getQoreContextRequiredValues({
      context,
      connectionFields: ['token'],
      optionFields: ['playlist'],
  ...
  event_info: {
    desc: 'YouTube New Playlist Video Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
        kind: { type: 'string' },
...
export default YouTubeNewPlaylistVideoTrigger;
