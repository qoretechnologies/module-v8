/**
 * BambooHR Apply Where Condition
 *
 * Client-side filtering and sorting of employee records using Qore WHERE conditions.
 * Since BambooHR's custom reports API lacks server-side filtering,
...
const evaluateCondition = (
  ...
  const fieldArg = args.find(
    (a) => typeof a === 'object' && a !== null && 'field' in a
  );
  const valueArg = args.find(
    (a) => typeof a === 'object' && a !== null && 'value' in a
  );
  ...
  const field = (fieldArg as { field: string }).field;
  const fieldValue = record[field];
  const compareValue =
    valueArg && typeof valueArg === 'object' && 'value' in valueArg
      ? (valueArg as { value: unknown }).value
      : undefined;
