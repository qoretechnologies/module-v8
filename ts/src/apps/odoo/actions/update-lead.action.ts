...
const updateLead = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      ...
      if (obj?.name) leadData.name = obj.name;
      if (obj?.type) leadData.type = obj.type;
      ...
      const success = await client.update('crm.lead', lead_id, leadData);
      ...
      throw new OdooError(`Failed to update lead: ${error.message || error}`);
    }
  },
  response_type: {
    type: 'hash',
