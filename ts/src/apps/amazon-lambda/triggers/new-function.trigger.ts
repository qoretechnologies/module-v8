...
const AWSLambdaNewFunctionTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    await pollCreatedItemsForTrigger({
      trigger_name: 'aws_lambda_new_function',
      uniqueField: 'function_name',
      getItems,
      update,
      should_stop,
    });
  ...
  get_example_event_data: async (context) => {
    ...
    const functions = await fetchLatestFunctions({
      access_key_id,
      secret_access_key,
      region: region || 'us-east-1',
    });
  ...
  event_info: {
    desc: 'AWS Lambda New Function Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
...
export default AWSLambdaNewFunctionTrigger;

const fetchLatestFunctions = async (options: {
  ...
  try {
    ...
    throw new AmazonLambdaError(`Failed to fetch latest functions: ${error.message || error}`);
  }
};
