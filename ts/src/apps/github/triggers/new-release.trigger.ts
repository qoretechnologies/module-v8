...
const githubNewReleaseTrigger = QoreAppCreator.createLocalizedTrigger({
  app: GITHUB_APP_NAME,
  action: 'new_release',
  ...
  webhook_register: async (context, url) => {
    ...
    if (!token || !owner || !repo) {
      throw new Error(
        'The following options are required to register new release event: token, owner, repo'
      );
    }
  ...
  webhook_deregister: async (context, _url, regInfo) => {
    ...
    if (!token || !owner || !repo) {
      throw new Error(
        'The following options are required to deregister new release event: token, owner, repo'
      );
    }
...
export default githubNewReleaseTrigger;
