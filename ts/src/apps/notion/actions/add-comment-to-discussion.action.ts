...
import { getNotionDiscussionsAllowedValues } from '../helpers/get-discussion-allowed-values';

const action = 'add_comment_to_discussion';
...
const options = {
  discussion_id: {
    type: 'string',
    required: true,
    get_allowed_values: getNotionDiscussionsAllowedValues,
  },
  text: {
    type: 'string',
    required: true,
...
const addCommentToDiscussion = QoreAppCreator.createLocalizedAction<typeof options>({
  app: NOTION_APP_NAME,
  action,
  action_code: EQoreAppActionCode.ACTION,
  options,
  api_function: async (obj, _opts, context) => {
    const { token, discussion_id, text } = getQoreContextRequiredValues({
      context: { ...context, opts: obj },
      connectionFields: ['token'],
      optionFields: ['discussion_id', 'text'],
...
export default addCommentToDiscussion;
