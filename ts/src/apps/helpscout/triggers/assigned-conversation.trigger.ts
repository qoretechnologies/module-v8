...
const trigger = 'assigned_conversation';
...
const HelpScoutAssignedConversation = QoreAppCreator.createLocalizedTrigger<typeof options>({
  action: trigger,
  ...
  webhook_register: async (context, url) => {
    ...
    const { webhookId } = await createHelpScoutWebhook({
      ...
      label: `Qorus Assigned Conversation Webhook`,
  ...
  event_info: {
    desc: 'Assigned Conversation data',
    type: HelpScoutConversationResponseType,
  },
});

export default HelpScoutAssignedConversation;
