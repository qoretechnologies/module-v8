...
const trigger = 'new_customer';

const options = {} satisfies TQoreOptions;

const HelpScoutNewCustomer = QoreAppCreator.createLocalizedTrigger<typeof options>({
  action: trigger,
  ...
  webhook_register: async (context, url) => {
    ...
    const { webhookId } = await createHelpScoutWebhook({
      ...
      label: `Qorus New Customer Webhook`,
    });
  ...
  event_info: {
    desc: 'New Customer data',
    type: HelpScoutCustomerWebhookResponseType,
  },
});

export default HelpScoutNewCustomer;
