...
const KlaviyoNewListProfileTrigger = QoreAppCreator.createLocalizedTrigger({
  app: KLAVIYO_APP_NAME,
  action: 'new_list_profile',
  ...
  event_function: async (context, update, should_stop) => {
    ...
    const getItems = () => {
      return fetchLatestEvents({ token, list });
    };

    await pollCreatedItemsForTrigger({
      trigger_name: 'klaviyo_new_list_profile',
      uniqueField: 'id',
      getItems,
      update,
      should_stop,
  ...
  get_example_event_data: async (context) => {
    ...
    const profiles = await fetchLatestEvents({ token, list });
...
export default KlaviyoNewListProfileTrigger;

const fetchLatestEvents = async (options: { token: string; list: string }) => {
  const { token, list } = options;

  try {
    const { listsApi } = getKlaviyoApis(token);

    const response = await listsApi.getProfileIdsForList(list, {
    ...
    throw new KlaviyoError(
      `Failed to fetch latest list profiles: ${getKlaviyoErrorMessage(error)}`
    );
  }
};
