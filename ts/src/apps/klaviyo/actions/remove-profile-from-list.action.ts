...
const removeProfileFromList = QoreAppCreator.createLocalizedAction<typeof options>({
  app: KLAVIYO_APP_NAME,
  action: 'remove_profile_from_list',
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      await apis.listsApi.removeProfilesFromList(list, {
        data: [
          {
            id: profile,
            type: 'profile',
      ...
      throw new KlaviyoError(
        `Failed to remove a profile from list: ${getKlaviyoErrorMessage(error)}`
      );
    }
  },
});

export default removeProfileFromList;
