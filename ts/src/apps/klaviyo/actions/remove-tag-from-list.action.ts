...
const removeTagFromList = QoreAppCreator.createLocalizedAction<typeof options>({
  app: KLAVIYO_APP_NAME,
  action: 'remove_tag_from_list',
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      await apis.tagsApi.removeTagFromLists(tag, {
        data: [
          {
            id: list,
            type: 'list',
      ...
      throw new KlaviyoError(`Failed to remove a tag from list: ${getKlaviyoErrorMessage(error)}`);
    }
  },
});
