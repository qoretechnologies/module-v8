...
const removeTagFromSegment = QoreAppCreator.createLocalizedAction<typeof options>({
  app: KLAVIYO_APP_NAME,
  action: 'remove_tag_from_segment',
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      await apis.tagsApi.removeTagFromSegments(tag, {
        data: [
          {
            id: segment,
            type: 'segment',
      ...
      throw new KlaviyoError(
        `Failed to remove a tag from segment: ${getKlaviyoErrorMessage(error)}`
      );
    }
  },
});

export default removeTagFromSegment;
