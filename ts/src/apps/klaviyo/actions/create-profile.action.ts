...
const options = {
  email: {
    type: 'string',
    required_groups: ['create_profile'],
  },
  phoneNumber: {
    type: 'string',
    required_groups: ['create_profile'],
...
const createProfile = QoreAppCreator.createLocalizedAction<typeof options>({
  ...
  api_function: async (obj, _opts, context) => {
    ...
    try {
      const response = await apis.profilesApi.createProfile({
        data: {
          type: 'profile',
          attributes: {
            ...(email && { email }),
      ...
      throw new KlaviyoError(`Failed to create profile: ${getKlaviyoErrorMessage(error)}`);
    }
  },
  response_type: {
    type: 'hash',
