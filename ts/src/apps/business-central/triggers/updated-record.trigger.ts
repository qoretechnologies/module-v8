...
const BusinessCentralUpdatedRecordTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    const getItems = () => {
      return fetchLatestUpdatedRecords({
        token,
        instance_type,
        filter,
        object,
    ...
    await pollCreatedItemsForTrigger({
      trigger_name: 'business_central_updated_record',
      uniqueField: 'id',
      getItems,
      update,
      should_stop,
  ...
  get_example_event_data: async (context) => {
    ...
    const records = await fetchLatestUpdatedRecords({
      token,
      instance_type,
      filter,
      object,
  ...
  event_info: {
    desc: 'Business Central Updated Record Trigger Event Info',
    type: {
      type: 'hash',
      fields: {
        id: { type: 'string' },
...
export default BusinessCentralUpdatedRecordTrigger;

const fetchLatestUpdatedRecords = async (options: {
  token: string;
  ...
  try {
    ...
    throw new BusinessCentralError(
      `Failed to fetch latest updated records for ${object}: ${error.message || error}`
    );
  }
};
