...
const AirtableUpdatedRecordTrigger = QoreAppCreator.createLocalizedTrigger({
  ...
  event_function: async (context, update, should_stop) => {
    ...
    const getItems = () => {
      return fetchLastUpdatedRecords(token, {
        baseId: base_id,
        tableId: table_id,
        lastPollTime,
        view,
    ...
    await pollCreatedItemsForTrigger({
      trigger_name: 'airtable_updated_record',
      uniqueField: 'id',
      getItems,
      updateLastPollTime,
      update,
  ...
  get_example_event_data: async (context) => {
    ...
    const records = await fetchLastUpdatedRecords(token, {
      baseId: base_id,
      tableId: table_id,
      view,
    });
  ...
  event_info: {
    desc: 'Airtable Updated Record Trigger Event Info',
    type: {
      type: 'hash',
    },
  },
...
const fetchLastUpdatedRecords = async (
  token: string,
  options: {
    baseId: string;
    tableId: string;
    ...
    await client
      .select({
        ...
        ...(options.lastPollTime && {
          filterByFormula: `LAST_MODIFIED_TIME() > "${options.lastPollTime}"`,
        }),
        maxRecords: pageSize,
        pageSize,
      })
...
export default AirtableUpdatedRecordTrigger;
