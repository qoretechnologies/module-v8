import { CatalogType, ITimePeriod, IUnitPriceOverride, TaxMode } from '@paddle/paddle-node-sdk';
import {
  EQoreAppActionCode,
  QoreAppCreator,
  TQoreOptions,
...
const createPrice = QoreAppCreator.createLocalizedAction<typeof options>({
  app: PADDLE_APP_NAME,
  action: 'create_price',
  action_code: EQoreAppActionCode.ACTION,
  options,
  api_function: async (obj, _opts, context) => {
    const { token, instance_type, description, product_id, unit_price } =
      getQoreContextRequiredValues({
        context: { ...context, opts: obj },
        optionFields: ['description', 'product_id', 'unit_price'],
        connectionFields: ['token', 'instance_type'],
        ErrorClass: PaddleError,
      });

    const { type, name, quantity, custom_data } = obj || {};
