...
import {
  ...
  GetCanvaImage,
  ListCanvaDesigns,
  ListCanvaThreadReplies,
  UpdateCanvaImage,
  UploadCanvaImage,
  UploadCanvaImageByUrl,
...
describe('Canva', () => {
  ...
  beforeAll(async () => {
    ...
    if (!refreshToken || !clientId || !clientSecret || !codeVerifier) {
      throw new Error(`
        Please set the CANVA_REFRESH_TOKEN, CANVA_CLIENT_ID, 
        CANVA_CLIENT_SECRET, and CANVA_CODE_VERIFIER environment variables.
      `);
    }
  ...
  describe('Should test allowed values', () => {
    it('Should get design values', async () => {
      const allowed_values = await getCanvaDesignAllowedValues(base_context);

      expect(allowed_values).toBeDefined();
      expect(allowed_values.length).toBeGreaterThan(0);
      expect(allowed_values[0].value).toBeDefined();

      design = allowed_values[0].value;
    });
  });

  let uploadedImageId: string | undefined;
  let urlUploadedImageId: string | undefined;
  let design: string | undefined;
  let thread: string | undefined;
  describe('Should test actions', () => {
    it('Should upload an asset', async () => {
      const action = UploadCanvaImage;
    ...
    it('Should upload image by url', async () => {
      const action = UploadCanvaImageByUrl;
      ...
      urlUploadedImageId = result.id;
    });

    it('Should delete the uploaded image by URL', async () => {
      const action = DeleteCanvaImage;
    ...
    it('Should create a thread', async () => {
      const action = CreateCanvaThread;
    ...
    it('Should list replies', async () => {
      const action = ListCanvaThreadReplies;
    ...
    describe('Should test triggers event example data', () => {
      it('Should get example event data for new design', async () => {
        const trigger = NewCanvaDesign;
      ...
      it('Should get example event data for new thread reply', async () => {
        const trigger = NewCanvaThreadReply;
...
const updateCanvaRefreshTokenVariable = async (refreshToken: string) => {
  const canvaRefreshTokenSecretName = process.env.CANVA_REFRESH_TOKEN_SECRET_NAME;
  ...
  if (!canvaRefreshTokenSecretName || !ghModuleRepoName || !ghModuleRepoOwner || !ghPatForSecrets) {
  ...
  try {
    ...
    await octokit.rest.actions.createOrUpdateRepoSecret({
      owner: ghModuleRepoOwner,
      repo: ghModuleRepoName,
      secret_name: canvaRefreshTokenSecretName,
    ...
    return {
      success: true,
      secretName: canvaRefreshTokenSecretName,
